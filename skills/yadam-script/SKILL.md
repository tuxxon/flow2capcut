...
| 방식 | 사용법 | 주의사항 |
|------|--------|---------|
| **레퍼런스 배치 (권장)** | AI: `app_start_ref_batch` 호출 / 또는 사용자가 앱에서 "Ref" → "일괄생성" → "생성시작" 클릭 | 스타일은 사전에 선택되어 있어야 함 |
| **씬 배치 (권장)** | AI: `app_start_scene_batch` 호출 / 또는 사용자가 앱에서 "생성시작" 클릭 | 레퍼런스 이미지가 먼저 완료되어야 함 |
| **개별 생성** | `app_generate_reference` 또는 `app_generate_scene`으로 한 건씩 | **7~15초 간격**을 두고 순차 요청해야 함 |

- **❌ 개별 생성을 동시에 여러 건 요청하면 전부 에러 발생** — 절대 병렬 호출하지 않는다
- AI가 개별 생성할 때는 한 건 완료 후 7~15초 대기 → 다음 건 순서로 진행
...
flow2capcut MCP: app_start_scene_batch({ styleId: "korean-ani" }) → 스타일 선택 + 씬 일괄 생성 시작
